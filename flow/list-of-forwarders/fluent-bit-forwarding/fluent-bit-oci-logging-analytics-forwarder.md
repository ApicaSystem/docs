# Fluent-bit OCI Logging Analytics Forwarder

### How to Forward logs from Apica Ascent to OCI Logging Analytics using Fluent-bit

#### 1. Introduction

This guide explains how to forward logs from Apica Ascent to OCI Logging Analytics using the **oci\_logging\_analytics** output plugin with Fluent-bit, and how to view them.
...
In this example we use the [HTTP](https://docs.apica.io/integrations/list-of-integrations/fluent-bit/fluent-bit-http-input-plugin) input plugin, Modify filter and the oci\_logging\_analytics output.
...
[OUTPUT]
    ...
    log_source_id     <LOG_SOURCE_OCID>
```

If you’re using the principal you should use the following output:
...
The region code should be the format of the following examples: **eu-stockholm-1**, **us-ashburn-1**, **eu-frankfurt-1**
...
1\.        Navigate to your OCI environment

2\.        Navigate to **Logging Analytics → Search**

3\.        Search for the log source name or filter by log group
...
If logs are not arriving in your end destination verify that the following are correct:
...
You could also try setting the buffering delay to 1-2min
